...
# GLOBAL VARIABLE /////////////////////////////////////////////////////////////
# control & guidance ----------------------------------------------------------
o_t_b = np.zeros((3,))    # target position    in drone body frame
k_t_b = np.zeros((3,))    # target orientation in drone body frame
...
def get_delta_yaw():
    ...
    k_t_b_proj = k_t_b[0:1]    # normal of target projected on {x_b, y_b} plane
    if k_t_b_proj[0] < 0:    # if drone is confronting marker
        psi = np.arccos(np.dot(-k_t_b_proj, i_b_b))
